      ...
      for(i3 in 1:length(seasontype))
      {
        for(i4 in 1:length(damped))
        {
          ...
          if(additive.only & (errortype[i1]=="M" | seasontype[i3]=="M"))
            next
          fit <- robetsmodel(y,errortype[i1],trendtype[i2],seasontype[i3],damped[i4],alpha,beta,gamma,phi,lower=lower,upper=upper,opt.crit=opt.crit,bounds=bounds,opt.initial.values=opt.initial.values,rob.start.initial.values=rob.start.initial.values,opt.sigma0=opt.sigma0,k=k,nmse=nmse,...,scale.estimator = scale.estimator)
...
robetsmodel <- function(y, errortype, trendtype, seasontype, damped,
                        ...
                        opt.initial.values = TRUE,rob.start.initial.values=TRUE,opt.sigma0=FALSE, k=NULL,scale.estimator,nmse)
{
  tsp.y <- tsp(y)
  if(is.null(tsp.y))
  ...
  if(rob.start.initial.values){
    initval <- robinitstate(y,errortype,trendtype,seasontype,scale.estimator)
  }else{
...
robinitstate <-  function(y, errortype, trendtype, seasontype, scale.estimator)
#robinitstate <-  function(y, errortype, trendtype, seasontype)
{
  ...
  if (seasontype != "N")
  {
    ...
    if (seasontype == "A") {
      ...
      if (errortype == "A")
        ...
        if (scale.estimator == "mad") sigma0 <- mad(y[1:startup] - ydt - init.seas)
        else if (scale.estimator == "Qn") sigma0 <- Qn(y[1:startup] - ydt - init.seas)
        else if (scale.estimator == "Sn") sigma0 <- Sn(y[1:startup] - ydt - init.seas)
        else if (scale.estimator == "Pn") sigma0 <- Pn(y[1:startup] - ydt - init.seas)
      else
        # if errortype = "M"
        # sigma0 <- mad((y[1:startup] - ydt - init.seas) / (ydt - init.seas))
        if (scale.estimator == "mad") sigma0 <- mad((y[1:startup] - ydt - init.seas) / (ydt - init.seas))
        else if (scale.estimator == "Qn") sigma0 <- Qn((y[1:startup] - ydt - init.seas) / (ydt - init.seas))
        else if (scale.estimator == "Sn") sigma0 <- Sn((y[1:startup] - ydt - init.seas) / (ydt - init.seas))
        else if (scale.estimator == "Pn") sigma0 <- Pn((y[1:startup] - ydt - init.seas) / (ydt - init.seas))
    } else{
      init.seas <- rowMedians(matrix(y[1:startup] / ydt, nrow = m))
      init.seas <-
    ...
    # non-seasonal model
  {
    ...
    if (errortype == "A")
      ...
      if (scale.estimator == "mad") sigma0 <- mad(y[1:startup] - ydt)
      else if (scale.estimator == "Qn") sigma0 <- Qn(y[1:startup] - ydt)
      else if (scale.estimator == "Sn") sigma0 <- Sn(y[1:startup] - ydt)
      else if (scale.estimator == "Pn") sigma0 <- Pn(y[1:startup] - ydt)
    else
      # if errortype = "M"
      # sigma0 <- mad((y[1:startup] - ydt) / ydt)
      if (scale.estimator == "mad") sigma0 <- mad((y[1:startup] - ydt) / ydt)
      else if (scale.estimator == "Qn") sigma0 <- Qn((y[1:startup] - ydt) / ydt)
      else if (scale.estimator == "Sn") sigma0 <- Sn((y[1:startup] - ydt) / ydt)
      else if (scale.estimator == "Pn") sigma0 <- Pn((y[1:startup] - ydt) / ydt)
  }
